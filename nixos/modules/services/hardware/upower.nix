...

{
  ...
  config = mkIf cfg.enable {
    ...
    text = ''
      [UPower]
      EnableWattsUpPro=${if cfg.enableWattsUpPro then "true" else "false"}
      NoPollBatteries=${if cfg.noPollBatteries then "true" else "false"}
      IgnoreLid=${if cfg.ignoreLid then "true" else "false"}
      UsePercentageForPolicy=${if cfg.usePercentageForPolicy then "true" else "false"}
      PercentageLow=${toString cfg.percentageLow}
      PercentageCritical=${toString cfg.percentageCritical}
      PercentageAction=${toString cfg.percentageAction}
      TimeLow=${toString cfg.timeLow}
      TimeCritical=${toString cfg.timeCritical}
      TimeAction=${toString cfg.timeAction}
      CriticalPowerAction=${toString cfg.criticalPowerAction}
    '';
  };
